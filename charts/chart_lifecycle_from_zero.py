...
class ChartLifecycleFromZero(BaseChart):
    ...
    def __init__(self, chart_id='chart_lifecycle_from_zero', default_show_labels=False, **kwargs):
    ...
    def _generate_chart_selectors_html(self) -> str:
        ...
        <div class="chart-selectors" style="display: flex; gap: 20px; margin-bottom: 12px; padding: 10px; background: #f8f9fa; border-radius: 8px; border: 1px solid #e9ecef; flex-wrap: wrap; align-items: center;">
            <div class="selector-group" style="display: flex; align-items: center; gap: 8px;">
                <label style="font-weight: 500; font-size: 13px; color: #495057;">Период:</label>
                <select id="{self.chart_id}_date_from" onchange="update{self.chart_id}()" style="padding: 4px 8px; border: 1px solid #ced4da; border-radius: 6px; font-size: 12px; background: white;"></select>
                <span style="color: #6c757d;">—</span>
                <select id="{self.chart_id}_date_to" onchange="update{self.chart_id}()" style="padding: 4px 8px; border: 1px solid #ced4da; border-radius: 6px; font-size: 12px; background: white;"></select>
            </div>
            <div class="selector-group" style="display: flex; align-items: center; gap: 8px;">
    ...
    def get_js_code(self):
        ...
        const numToMonthName_{self.chart_id} = {{
        ...
        // Флаг первой загрузки фильтра дат
        window.datesInitialized_{self.chart_id} = false;
        // Хранение первого периода в данных (для фильтрации "новых" магазинов)
        window.firstPeriodKey_{self.chart_id} = null;

        // Инициализация локального фильтра дат
        function initDateFilter_{self.chart_id}(data) {{
            const fromSelect = document.getElementById('{self.chart_id}_date_from');
            const toSelect = document.getElementById('{self.chart_id}_date_to');
            if (!fromSelect || !toSelect) return;

            // Собираем все уникальные периоды из rawData (не filteredData!)
            const periods = new Set();
            const rawData = window.rawData || data;
            rawData.forEach(row => {{
            ...
            // Сохраняем первый период для фильтрации магазинов
            window.firstPeriodKey_{self.chart_id} = sortedPeriods[0];

            // Только при первой инициализации заполняем селекторы
            if (!window.datesInitialized_{self.chart_id}) {{
                fromSelect.innerHTML = '';
                toSelect.innerHTML = '';

                sortedPeriods.forEach(pk => {{
                    const year = Math.floor((pk - 1) / 12);
                    const month = ((pk - 1) % 12) + 1;
                    const label = `01.${{String(month).padStart(2, '0')}}.${{year}}`;
                ...
                // По умолчанию весь диапазон
                fromSelect.value = sortedPeriods[0];
                toSelect.value = sortedPeriods[sortedPeriods.length - 1];

                window.datesInitialized_{self.chart_id} = true;
            }}
        }}

        // Получить локально отфильтрованные данные
        function getLocalFilteredData_{self.chart_id}() {{
            const data = window.rawData || [];
            const fromSelect = document.getElementById('{self.chart_id}_date_from');
            const toSelect = document.getElementById('{self.chart_id}_date_to');

            if (!fromSelect || !toSelect) return data;

            const fromPk = parseInt(fromSelect.value) || 0;
            const toPk = parseInt(toSelect.value) || 999999;
            ...
            return data.filter(row => {{
                ...
                const pk = year * 12 + month;
                return pk >= fromPk && pk <= toPk;
            }});
        }}

        // Локальный фильтр магазинов - выбираем по умолчанию только "новые" магазины
        function updateStoreSelector_{self.chart_id}(stores, storeFirstPeriods) {{
            ...
            const firstPeriodInData = window.firstPeriodKey_{self.chart_id};
            ...
            stores.sort(naturalSort_{self.chart_id}).forEach(store => {{
                ...
                if (window.storesInitialized_{self.chart_id}) {{
                    // После первой загрузки - сохраняем выбор пользователя
                    option.selected = currentSelected.includes(store);
                }} else {{
                    // При первой загрузке - выбираем только магазины, открывшиеся ПОСЛЕ первого периода
                    const storeFirstPeriod = storeFirstPeriods[store] || 0;
                    option.selected = storeFirstPeriod > firstPeriodInData;
                }}
        ...
        function buildLifecycleData_{self.chart_id}(data, metricField) {{
            ...
            data.forEach(row => {{
                ...
                const monthName = row['Месяц'];
                const month = monthNameToNum_{self.chart_id}[monthName] || 0;
                // Для метрики "Число чеков" используем хелпер (группировка по магазину)
                const metricValue = metricField === 'Число чеков'
                    ? getChecksValue(row, 'Магазин')
            ...
            const storeFirstPeriods = {{}};  // Первый период для каждого магазина
            ...
            Object.entries(storeMonthly).forEach(([store, periods]) => {{
                ...
                // Сохраняем первый период магазина (для фильтрации "новых")
                storeFirstPeriods[store] = sortedKeys[firstPeriodIdx];
            ...
            return {{
                ...
                storeFirstPeriods: storeFirstPeriods,
        ...
        function update{self.chart_id}() {{
            ...
            const showLabels = document.getElementById('{self.chart_id}_show_labels')?.checked ?? false;

            // Инициализируем локальный фильтр дат
            initDateFilter_{self.chart_id}(window.rawData);

            // Используем локально отфильтрованные данные (независимо от глобального фильтра)
            const data = getLocalFilteredData_{self.chart_id}();
            const result = buildLifecycleData_{self.chart_id}(data, metricField);
            const lifecycleData = result.stores;
            const storeFirstPeriods = result.storeFirstPeriods;
            ...
            // Обновляем локальный селектор магазинов (передаём первые периоды для фильтрации)
            updateStoreSelector_{self.chart_id}(allStores, storeFirstPeriods);
            ...
            // Аннотации для отображения значений
            const annotations = [];
            if (showLabels) {{
                stores.forEach((store, idx) => {{
                    const lc = lifecycleData[store];
                    lc.forEach((d, i) => {{
                        ...
                        annotations.push({{
                            x: d.monthFromStart,
                            y: yVal,
                            text: isPercent ? yVal + '%' : yVal.toLocaleString('ru-RU'),
                            showarrow: false,
                            font: {{
                                size: 8,
                                color: storeColors_{self.chart_id}[idx % storeColors_{self.chart_id}.length]
