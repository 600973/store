...
class DashboardEngine:
    ...
    def __init__(self, df: pd.DataFrame, tabs: dict = None, layout: GridLayout = None,
        ...
        self.filters = GlobalFilters(df, filter_config=filter_config)
    ...
    def generate_html(self, output_file: str = 'dashboard.html') -> str:
        ...
        # Замена для каждой вкладки
        for tab_id, tab_html in tabs_html.items():
            placeholder = f'{{{{CHARTS_HTML_{tab_id.upper()}}}}}'
            print(f"Замена {placeholder} на {len(tab_html)} символов")
            html_content = html_content.replace(placeholder, tab_html)
        
        # Проверка незамененных плейсхолдеров
        if '{{CHARTS_HTML' in html_content:
            print("WARNING: Остались незамененные плейсхолдеры CHARTS_HTML")
