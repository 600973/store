...
# ============================================================
# Этот код добавляется в notebook store_analytics.ipynb
# ============================================================

"""
📐 ОПРЕДЕЛЕНИЕ ОПТИМАЛЬНОЙ ПЛОЩАДИ МАГАЗИНА

5 методов анализа:
1. Анализ производительности на м² (максимизация выручки/м² и прибыли/м²)
2. Нелинейная регрессия (поиск точки убывающей отдачи)
3. Предельная эффективность (маржинальный анализ)
4. Кластеризация + бенчмаркинг (лидеры по сегментам)
5. Оптимизация с учетом списаний (реальная прибыль/м²)
"""

# ============================================================
# 4.0 ЗАГРУЗКА ДАННЫХ О ПЛОЩАДИ И СПИСАНИЯХ
# ============================================================
...
# ВАЖНО: Укажите путь к файлу с площадями магазинов
# Формат: первая колонка - "Магазин" или адрес, вторая - "Площадь"
STORE_AREA_FILE = 'store.xlsx'  # ← ИЗМЕНИТЕ НА ВАШЕ ИМЯ ФАЙЛА

try:
    # Загрузка файла с площадями
    df_area = pd.read_excel(STORE_AREA_FILE)

    # Определяем название колонок автоматически
    ...
    # Предполагаем: 1-я колонка = адрес/магазин, 2-я = площадь
    df_area.columns = ['Магазин_адрес', 'Площадь_м2'] + col_names[2:]

    # Если в данных есть колонка "Списания", загружаем её
    if len(df_area.columns) > 2:
        has_writeoffs = True
        df_area.columns = ['Магазин_адрес', 'Площадь_м2', 'Списания'] + col_names[3:]
        print(f"✓ Найдены данные о списаниях")
    else:
        has_writeoffs = False
        print("⚠️ Данные о списаниях не найдены (будут рассчитаны только методы 1-4)")
    ...
    print(f"\nИнструкция:")
    print(f"1. Положите файл с площадями в ту же папку, что и notebook")
    print(f"2. Измените переменную STORE_AREA_FILE на имя вашего файла")
    print(f"3. Формат файла:")
    print(f"   Колонка 1: Адрес магазина (должен совпадать с 'Магазин' в основных данных)")
    print(f"   Колонка 2: Площадь (м²)")
    print(f"   Колонка 3: Списания (опционально)")
    raise
...
# Если не получается, пробуем по адресу
df_area['Магазин'] = df_area['Магазин_адрес']

# Мерж данных
store_with_area = store_summary.merge(
    df_area[['Магазин', 'Площадь_м2'] + (['Списания'] if has_writeoffs else [])],
    on='Магазин',
    how='left'
)
...
if missing_area > 0:
    print(f"\n⚠️ ВНИМАНИЕ: Для {missing_area} магазинов не найдена площадь!")
    print("Проверьте, что названия магазинов совпадают в обоих файлах:")
    print("\nМагазины без площади:")
    print(store_with_area[store_with_area['Площадь_м2'].isna()]['Магазин'].tolist())
    print("\nДоступные адреса в файле площадей:")
    print(df_area['Магазин_адрес'].tolist())

    # Попытка автоматического сопоставления
    print("\n🔧 Попытка автоматического сопоставления...")
    # Извлекаем номер магазина из обеих таблиц
    ...
    # Повторный мерж по номеру
    store_with_area = store_with_area.drop(columns=['Площадь_м2'] + (['Списания'] if has_writeoffs else []), errors='ignore')
    store_with_area = store_with_area.merge(
        df_area[['temp_id', 'Площадь_м2'] + (['Списания'] if has_writeoffs else [])],
        on='temp_id',
        how='left'
    )
...
print(f"\n✓ Объединение завершено: {len(store_with_area)} магазинов с площадью")
...
if has_writeoffs:
    store_with_area['Чистая_прибыль'] = store_with_area['Валовая_прибыль'] - store_with_area['Списания']
    store_with_area['Чистая_прибыль_на_м2'] = store_with_area['Чистая_прибыль'] / store_with_area['Площадь_м2']
    store_with_area['Loss_Rate_%'] = (store_with_area['Списания'] / store_with_area['Выручка'] * 100).round(2)
...
y_profit = store_with_area['Валовая_прибыль'].values
...
# 3. Логистическая кривая (аппроксимация через полином 3-й степени)
...
if r2_poly > r2_linear and len(lr_poly.coef_) >= 3:
    ...
    if c < 0:  # Парабола вниз (есть максимум)
        ...
        # Точка, где предельная выручка падает на 50% от макс.
...
# Сортируем для правильного отображения линий
...
# Расчёт предельной выручки (изменение выручки на изменение площади)
...
# Находим точку, где предельная выручка начинает устойчиво падать
...
# ============================================================
# МЕТОД 5: ОПТИМИЗАЦИЯ С УЧЁТОМ СПИСАНИЙ
# ============================================================

if has_writeoffs:
    print("\n" + "="*80)
    print("💰 МЕТОД 5: ОПТИМИЗАЦИЯ С УЧЁТОМ СПИСАНИЙ")
    print("="*80)
    print("Цель: Найти площадь с максимальной ЧИСТОЙ прибылью/м² после вычета списаний\n")

    # Топ по чистой прибыли/м²
    top_by_net_profit = store_with_area.nlargest(5, 'Чистая_прибыль_на_м2')[
        ['Магазин', 'Площадь_м2', 'Валовая_прибыль', 'Списания', 'Чистая_прибыль', 'Чистая_прибыль_на_м2', 'Loss_Rate_%']
    ]

    print("🏆 ТОП-5 по чистой прибыли/м² (после списаний):")
    display(top_by_net_profit.round(0))

    # Оптимальная площадь
    optimal_area_method5 = top_by_net_profit['Площадь_м2'].mean()

    print(f"\n💡 ВЫВОД МЕТОДА 5:")
    print(f"  • Оптимальная площадь (макс. чистая прибыль/м²): {optimal_area_method5:.0f} м²")
    print(f"  • Средний Loss Rate в топ-5: {top_by_net_profit['Loss_Rate_%'].mean():.1f}%")
    print(f"  • Рекомендуемый диапазон: {top_by_net_profit['Площадь_м2'].min():.0f}-{top_by_net_profit['Площадь_м2'].max():.0f} м²")

    # Анализ: влияние площади на списания
    correlation_area_loss = store_with_area[['Площадь_м2', 'Loss_Rate_%']].corr().iloc[0, 1]
    print(f"\n📊 Корреляция площади и % списаний: {correlation_area_loss:.3f}")
    if correlation_area_loss > 0.3:
        print("  ⚠️ Большие магазины имеют больше списаний!")
    elif correlation_area_loss < -0.3:
        print("  ✓ Большие магазины имеют меньше списаний (эффект масштаба)")
    else:
        print("  → Площадь слабо влияет на % списаний")

    # Визуализация
    fig = make_subplots(
        rows=1, cols=2,
        subplot_titles=['Валовая vs Чистая прибыль/м²', 'Влияние площади на % списаний']
    )

    # График 1: Сравнение валовой и чистой прибыли
    fig.add_trace(
        go.Bar(x=store_with_area['Магазин'], y=store_with_area['Прибыль_на_м2'],
               name='Валовая прибыль/м²', marker_color='lightblue'),
        row=1, col=1
    )
    fig.add_trace(
        go.Bar(x=store_with_area['Магазин'], y=store_with_area['Чистая_прибыль_на_м2'],
               name='Чистая прибыль/м²', marker_color='darkblue'),
        row=1, col=1
    )

    # График 2: Площадь vs % списаний
    fig.add_trace(
        go.Scatter(x=store_with_area['Площадь_м2'], y=store_with_area['Loss_Rate_%'],
                   mode='markers', marker=dict(size=10, color=store_with_area['Чистая_прибыль_на_м2'],
                                               colorscale='RdYlGn', showscale=True),
                   text=store_with_area['Магазин'], name='Магазины'),
        row=1, col=2
    )

    # Линия тренда
    from scipy.stats import linregress
    slope, intercept, r_value, p_value, std_err = linregress(store_with_area['Площадь_м2'], store_with_area['Loss_Rate_%'])
    line_x = np.array([store_with_area['Площадь_м2'].min(), store_with_area['Площадь_м2'].max()])
    line_y = slope * line_x + intercept
    fig.add_trace(
        go.Scatter(x=line_x, y=line_y, mode='lines', name=f'Тренд (R²={r_value**2:.2f})',
                   line=dict(dash='dash', color='red')),
        row=1, col=2
    )

    fig.update_layout(height=500, title_text='💰 Метод 5: Анализ с учётом списаний', showlegend=True)
    fig.update_xaxes(title_text="Магазин", row=1, col=1)
    fig.update_xaxes(title_text="Площадь (м²)", row=1, col=2)
    fig.update_yaxes(title_text="Прибыль/м² (руб)", row=1, col=1)
    fig.update_yaxes(title_text="% списаний", row=1, col=2)
    fig.show()
    ...
    print("💰 МЕТОД 5: ОПТИМИЗАЦИЯ С УЧЁТОМ СПИСАНИЙ")
    print("="*80)
    print("⚠️ Данные о списаниях не предоставлены - метод пропущен")
    optimal_area_method5 = None
...
results = {
    ...
    'Оптимальная площадь (м²)': [
        ...
        optimal_area_method5 if has_writeoffs else None
    ],
    'Обоснование': [
        ...
        'Максимум чистой прибыли/м²' if has_writeoffs else 'Нет данных'
    ]
}
